...
def aggregate_results(results: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = results.groupby("box_size")[metrics].agg(["mean", "std"]).reset_index()
    summary.columns = [
...
def main() -> None:
    ...
    for box_size in BOX_SIZES:
        box_df = df[df["box_size"] == box_size].copy()
        print(f"\n=== Box size: {box_size} ===")
        print(f"Windows: {len(box_df)}, experiments: {box_df[GROUP_COL].nunique()}")
        print("Experiment labels:")
        print(box_df.groupby(GROUP_COL)[LABEL_COL].first().value_counts().to_string())

        for fold, (train_idx, test_idx) in enumerate(make_stratified_group_folds(box_df), start=1):
            train_df = box_df.loc[train_idx].copy()
            test_df = box_df.loc[test_idx].copy()

            overlap = set(train_df[GROUP_COL]) & set(test_df[GROUP_COL])
            if overlap:
                raise ValueError(f"Data leakage in box size {box_size}, fold {fold}: {sorted(overlap)}")

            row = evaluate_fold(box_size, fold, train_df, test_df)
            all_rows.append(row)
            print(
    ...
    output = fold_results.merge(summary, on="box_size", suffixes=("", "_summary"))
    output.to_csv(BOX_SIZE_CV_RESULTS_CSV, index=False)

    print(f"\nSaved box-size cross-validation results to {BOX_SIZE_CV_RESULTS_CSV}")
